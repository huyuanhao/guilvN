...
public final class BridgeInterceptor implements Interceptor {
    ...
    public Response intercept(Interceptor.Chain chain) throws IOException {
        ...
        if (request.header(HttpConstant.HOST) == null) {
            newBuilder.header(HttpConstant.HOST, Util.hostHeader(request.url(), false));
        }
        ...
        if (z && "gzip".equalsIgnoreCase(proceed.header("Content-Encoding")) && HttpHeaders.hasBody(proceed)) {
            oOOO0O00 oooo0o00 = new oOOO0O00(proceed.body().source());
            request2.headers(proceed.headers().newBuilder().removeAll("Content-Encoding").removeAll("Content-Length").build());
            request2.body(new RealResponseBody(proceed.header("Content-Type"), -1, C9143oooO0oOo.OooO00o(oooo0o00)));
        }
