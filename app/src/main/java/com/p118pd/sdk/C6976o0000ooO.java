package com.p118pd.sdk;

/* renamed from: com.pd.sdk.o0000ooO  reason: case insensitive filesystem */
public class C6976o0000ooO {
    public static final int[] OooO00o = new int[0];

    /* renamed from: OooO00o  reason: collision with other field name */
    public static final long[] f19182OooO00o = new long[0];

    /* renamed from: OooO00o  reason: collision with other field name */
    public static final Object[] f19183OooO00o = new Object[0];

    public static int OooO00o(int i) {
        for (int i2 = 4; i2 < 32; i2++) {
            int i3 = (1 << i2) - 12;
            if (i <= i3) {
                return i3;
            }
        }
        return i;
    }

    public static boolean OooO00o(Object obj, Object obj2) {
        return obj == obj2 || (obj != null && obj.equals(obj2));
    }

    public static int OooO0O0(int i) {
        return OooO00o(i * 4) / 4;
    }

    public static int OooO0OO(int i) {
        return OooO00o(i * 8) / 8;
    }

    public static int OooO00o(int[] iArr, int i, int i2) {
        int i3 = i - 1;
        int i4 = 0;
        while (i4 <= i3) {
            int i5 = (i4 + i3) >>> 1;
            int i6 = iArr[i5];
            if (i6 < i2) {
                i4 = i5 + 1;
            } else if (i6 <= i2) {
                return i5;
            } else {
                i3 = i5 - 1;
            }
        }
        return i4 ^ -1;
    }

    public static int OooO00o(long[] jArr, int i, long j) {
        int i2 = i - 1;
        int i3 = 0;
        while (i3 <= i2) {
            int i4 = (i3 + i2) >>> 1;
            long j2 = jArr[i4];
            if (j2 < j) {
                i3 = i4 + 1;
            } else if (j2 <= j) {
                return i4;
            } else {
                i2 = i4 - 1;
            }
        }
        return i3 ^ -1;
    }
}
