        ...
        public void run() {
            ...
            try {
                ...
                if (runnable != null) {
                    ...
                    FLog.m894v(ConstrainedExecutorService.TAG, "%s: Worker has nothing to run", ConstrainedExecutorService.this.mName);
                }
                if (isEmpty) {
                    FLog.m895v(ConstrainedExecutorService.TAG, "%s: worker finished; %d workers left", ConstrainedExecutorService.this.mName, Integer.valueOf(atomicInteger.decrementAndGet()));
                }
            } finally {
                ...
                if (!ConstrainedExecutorService.this.mWorkQueue.isEmpty()) {
                    ...
                    FLog.m895v(ConstrainedExecutorService.TAG, "%s: worker finished; %d workers left", ConstrainedExecutorService.this.mName, Integer.valueOf(decrementAndGet));
                }
            }
        }
    }
    ...
    private void startWorkerIfNeeded() {
        ...
        while (i < this.mMaxConcurrency) {
            ...
            if (this.mPendingWorkers.compareAndSet(i, i2)) {
                FLog.m896v(TAG, "%s: starting worker %d of %d", this.mName, Integer.valueOf(i2), Integer.valueOf(this.mMaxConcurrency));
                this.mExecutor.execute(this.mTaskRunner);
                return;
            }
            FLog.m894v(TAG, "%s: race in startWorkerIfNeeded; retrying", this.mName);
    ...
    public void execute(Runnable runnable) {
        if (runnable == null) {
            ...
            if (size > i && this.mMaxQueueSize.compareAndSet(i, size)) {
                FLog.m895v(TAG, "%s: max pending work in queue = %d", this.mName, Integer.valueOf(size));
            }
            startWorkerIfNeeded();
        } else {
