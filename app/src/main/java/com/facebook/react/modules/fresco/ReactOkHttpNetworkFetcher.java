...
public class ReactOkHttpNetworkFetcher extends OkHttpNetworkFetcher {
    public static final String TAG = "ReactOkHttpNetworkFetcher";
    ...
    public void fetch(OkHttpNetworkFetcher.OkHttpNetworkFetchState okHttpNetworkFetchState, NetworkFetcher.Callback callback) {
        ...
        fetchWithRequest(okHttpNetworkFetchState, callback, new Request.Builder().cacheControl(new CacheControl.Builder().noStore().build()).url(uri.toString()).headers(Headers.m14260of(headers)).get().build());
    }
}
