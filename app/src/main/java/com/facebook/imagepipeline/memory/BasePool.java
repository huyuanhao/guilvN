...
public abstract class BasePool<V> implements Pool<V> {
    ...
    private void logStats() {
        if (FLog.isLoggable(2)) {
            FLog.m897v(this.TAG, "Used = (%d, %d); Free = (%d, %d)", Integer.valueOf(this.mUsed.mCount), Integer.valueOf(this.mUsed.mNumBytes), Integer.valueOf(this.mFree.mCount), Integer.valueOf(this.mFree.mNumBytes));
        }
    }
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:33:0x009d, code lost:
        com.facebook.common.logging.FLog.m895v(r5.TAG, "get (alloc) (object, size) = (%x, %s)", java.lang.Integer.valueOf(java.lang.System.identityHashCode(r0)), java.lang.Integer.valueOf(r6));
     */
    ...
    public synchronized Bucket<V> getBucket(int i) {
        ...
        if (bucket == null) {
            if (this.mAllowNewBuckets) {
                if (FLog.isLoggable(2)) {
                    FLog.m894v(this.TAG, "creating new bucket %s", Integer.valueOf(i));
                }
    ...
    public void release(V v) {
        ...
        synchronized (this) {
            ...
            if (!this.mInUseValues.remove(v)) {
                FLog.m871e(this.TAG, "release (free, value unrecognized) (object, size) = (%x, %s)", Integer.valueOf(System.identityHashCode(v)), Integer.valueOf(bucketedSizeForValue));
                free(v);
                ...
                if (bucketIfPresent != null && !bucketIfPresent.isMaxLengthExceeded() && !isMaxSizeSoftCapExceeded()) {
                    if (isReusable(v)) {
                        ...
                        if (FLog.isLoggable(2)) {
                            FLog.m895v(this.TAG, "release (reuse) (object, size) = (%x, %s)", Integer.valueOf(System.identityHashCode(v)), Integer.valueOf(bucketedSizeForValue));
                        }
                    }
                }
                ...
                if (FLog.isLoggable(2)) {
                    FLog.m895v(this.TAG, "release (free) (object, size) = (%x, %s)", Integer.valueOf(System.identityHashCode(v)), Integer.valueOf(bucketedSizeForValue));
                }
                free(v);
                this.mUsed.decrement(sizeInBytes);
                this.mPoolStatsTracker.onFree(sizeInBytes);
    ...
    public synchronized void trimToSize(int i) {
        ...
        if (min > 0) {
            if (FLog.isLoggable(2)) {
                FLog.m896v(this.TAG, "trimToSize: TargetSize = %d; Initial Size = %d; Bytes to free = %d", Integer.valueOf(i), Integer.valueOf(this.mUsed.mNumBytes + this.mFree.mNumBytes), Integer.valueOf(min));
            }
            logStats();
            ...
            logStats();
            if (FLog.isLoggable(2)) {
                FLog.m895v(this.TAG, "trimToSize: TargetSize = %d; Final Size = %d", Integer.valueOf(i), Integer.valueOf(this.mUsed.mNumBytes + this.mFree.mNumBytes));
            }
        }
    }
